...
apk update
apk add --no-cache alpine-sdk python3-dev libffi-dev openssl-dev libedit-dev ncurses-dev cmake musl-dev
...
"$cpython_bin" -m pip install "$tool_name==$tool_version" --no-binary :all:
