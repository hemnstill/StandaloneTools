...
apk update
apk add --no-cache gawk m4 libssh-dev libressl-dev libnfs-dev libarchive-dev cmake alpine-sdk linux-headers musl-dev mingw-w64-gcc git
...
cmake_command=$(printf 'cmake -DUSEWX=no %s -DCMAKE_EXE_LINKER_FLAGS="%s" -DCMAKE_BUILD_TYPE=Release .' \
  "$without_plugins" \
  "-l:libuchardet.a -l:libstdc++.a -L/usr/i686-w64-mingw32/lib -l:libgcc_s.a")
echo ">> $cmake_command"
eval "$cmake_command"
