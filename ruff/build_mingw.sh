...
find "./target/x86_64-pc-windows-gnu/release" -maxdepth 1 -type f \( -name "$tool_name.exe" -o -name "$tool_name_ty.exe" \) -exec cp -f {} "$release_version_dirpath/" \;
...
strip "$tool_name.exe"
strip "$tool_name_ty.exe"

{ printf '### %s
SHA-256: %s
SHA-256: %s

' "$self_toolset_name.tar.gz" "$(sha256sum $tool_name.exe)" "$(sha256sum $tool_name_ty.exe)"
